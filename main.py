        ...
        ScreenManager:
            Screen:
                MDBoxLayout:
                    ...
                    MDTextField:
                    	id: express
                    	hint_text: "Enter Number of digits for OTP"
                    	helper_text:"press round button down below to generate  OTP"
                ...
                MDNavigationDrawer:
                    ...
                    MDBoxLayout:
                        ...
                        Image:
                            source:"assets/images/otpgen.png"
                        ...
                        ScrollView:
                        	MDList:
                                ...
                                TwoLineIconListItem:
                                	...
                                	secondary_text: "tap to mail me"
                                	on_press: 
                                	    webbrowser.open("mailto:truly.away@gmail.com")
                                    font_style:"Caption"
                                    IconLeftWidget:
                                    	icon: "gmail"
                                ...
                                OneLineIconListItem:
                                    text: "GitHub"
                                	on_press: webbrowser.open("https://github.com/utkarsh-naman")
                                    IconLeftWidget:
                                    	icon: "github"
                                    	on_press: webbrowser.open("https://github.com/utkarsh-naman")
                                OneLineIconListItem:
...
<Donation>:
	...
	MDFlatButton:
		...
		on_press:
			app.copyupi()
	AsyncImage:
		source: "https://i.ibb.co/5rpDx3v/payun.png"
		allow_stretch: True
...
<Donors>:
	...
	MDFillRoundFlatButton:
		text:"Check Updated Donor list "
		
		pos_hint: {'center_x': 0.5, 'center_y' : 0.2}
		on_press: webbrowser.open("https://docs.google.com/document/d/12EycvA9HVP4Znik2v_1mw0x5gaLRDvmspit8Dr2XuAE/edit?usp=drivesdk")
	...
	ScrollView:
		MDList:
			TwoLineIconListItem:
			    ...
			    on_press: webbrowser.open("https://drive.google.com/drive/folders/13cYcX7xFQ0mK62HaWar1yvWninEGRO7e")
			    IconLeftWidget:
			        icon: "android"
                    on_press: webbrowser.open("https://drive.google.com/drive/folders/13cYcX7xFQ0mK62HaWar1yvWninEGRO7e")
...
<History>:	
	...
	MDFillRoundFlatButton:
		...
		on_press:
			app.history_music(root)
			app.show_history(root)
...
class TTE(MDApp):
    ...
    def build(self):
        themed = sqlite3.connect('repos/database/theme_database.db')
        
        ct = themed.cursor()
        ct.execute("SELECT * FROM theme")
        
        records1 = ct.fetchall()
        style = ""
        for record in records1:
        	style = f'{record[0]}'
        	
        palette = sqlite3.connect('repos/database/palette_database.db')
        cp = palette.cursor()
        cp.execute("SELECT * FROM palette")
        
        records2 = cp.fetchall()
        palette = ""
        for record in records2:
        	palette = f'{record[0]}'
        
        accented = sqlite3.connect('repos/database/accent_database.db')
        ca = accented.cursor()
        ca.execute("SELECT * FROM accent")
        
        records3 = ca.fetchall()
        accent = ""
        ...
        self.theme_cls.theme_style = f"{style}"
        ...
        Loader.error_image="assets/images/upierror.png"
    ...
    def save_theme(self):
    	os.remove("repos/database/theme_database.db")
    	themed = sqlite3.connect('repos/database/theme_database.db')
    	ct = themed.cursor()
    	ct.execute("""CREATE TABLE if not exists theme(last_theme text)""")
    	ct.execute("INSERT INTO theme VALUES (:first)",{'first': self.theme_cls.theme_style })
    ...
    def save_palette(self):
    	os.remove("repos/database/palette_database.db")
    	palette = sqlite3.connect('repos/database/palette_database.db')
    	cp = palette.cursor()
    	cp.execute("""CREATE TABLE if not exists palette(Last_palette text)""")    	
    	cp.execute("INSERT INTO palette VALUES (:first)",{'first': self.theme_cls.primary_palette })
    	
    	palette.commit()
    	
    	palette.close()
    	
    def save_accent(self):
    	os.remove("repos/database/accent_database.db")
    	accented = sqlite3.connect('repos/database/accent_database.db')
    	ca = accented.cursor()
    	ca.execute("""CREATE TABLE if not exists accent(last_accent text)""")
    	ca.execute("INSERT INTO accent VALUES (:first)",{'first': self.theme_cls.accent_palette })
    ...
    def dialgclose(self,root):
    	 sound2= SoundLoader.load("assets/audio/enter.wav")
    	 if sound2:
        	sound2.loop= False
        	sound2.play()
    	 self.dialg.dismiss()
...
#16	
        ...
        self.dialg=MDDialog(title="Close App?", radius=(50,50,50,50), text="Do you want to exit?", size_hint=(0.8, 0.5), buttons=[end_button, no_button])
    ...
    def  donatepopup(self):
    	...
    	checker= sqlite3.connect('repos/database/donatepop.db')
    	ch = checker.cursor()
    	ch.execute("SELECT * FROM closer")
    	
    	records0 = ch.fetchall()
    	val = ""
    	...
    	if val == '0':   	    
    	    ...
    	    if sound1:
    	    	...
    	    	sound1.play()
    	    self.dialog1.open()
    	    os.remove("repos/database/donatepop.db")
    	    checker = sqlite3.connect('repos/database/donatepop.db')
    	    ch = checker.cursor()
    	    ch.execute("""CREATE TABLE if not exists closer(counter text)""")
    	    ch.execute("INSERT INTO closer VALUES (:first)",{'first': 1 })
    	    checker.commit()
    	    checker.close()
    ...
    def close_music(self, root):
    	sound=SoundLoader.load("assets/audio/enter.wav")
    	if sound:
    		sound.loop=False
    		sound.play()	
    def trash_history(self, root):
    	sound = SoundLoader.load("assets/audio/woosh.wav")
    	if sound:
    		sound.loop=False
    		sound.play()
    ...
    def copy_view(self):
    	...
    	self.dialog1 = MDDialog(title="Redirect Features", text="•Added copy result feature                                                                                            •Added email feature                                                                                            •Added link of latest apk                                                                                       •Added link of YouTube channel feature                                                                                                                                       •Added links in YouTube section feature                                                                                                                                                                   ", size_hint=[0.8, 0.3], buttons=[close_button])
    	self.dialog1.open()
    ...
    def ui_view(self):
    	...
    	self.dialog1 = MDDialog(title="UI CHANGELOG", text="•Now you can save theme and also reset it to default                                                                                                                                                                                                                                                •Press back button to exit                                                                                                                                                                                           •Added confirmation box on exit                                                                                                                                                                                         •Added colored buttons                                                                                                                                                                                      •Customized buttons in sync with Theme                                                                                                                                                                                                                   •Changed Result Box colour                                                                                                             •Added earlier donor section                                                                                                                                                        ", size_hint=[0.8, 0.3], buttons=[close_button])    	
    	self.dialog1.open()
...
#16	
        ...
        if sound1:
        	sound1.loop= False
        	sound1.play()
        self.dialog=MDDialog(title="Python Expression", radius=(50,50,50,50), text="", size_hint=(0.8, 0.5), buttons=[close_button, copy_button])
...
#16	
        ...
        if sound1:
        	sound1.loop= False
        	sound1.play()
        self.dialog=MDDialog(title="Java Expression", radius=(50,50,50,50), text="", size_hint=(0.8, 0.5), buttons=[close_button, copy_button])
...
#16	
        ...
        if exp == "":        	
            ...
            try:
        	    self.dialog.text=f"{random.randint((10**(int(eval(exp))-1)), 10**(int(eval(exp)))-1)}"
            except NameError:
            	self.dialog.text="Check your expression again"        	
            except ValueError:
        	    self.dialog.text="Number of digits should be limited to 9"
            except SyntaxError:
    ...
    def history_music(self,root):
    	sound = SoundLoader.load("assets/audio/load.wav")
    	if sound:
    		sound.loop= False
    		sound.play()
